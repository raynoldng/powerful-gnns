...
class GraphDS(nn.Module):
    ...
    def __init__(
        ...
        """
            ...
            num_mlp_layers: number of layers in mlps readout layer (EXCLUDING the input layer)
        ...
        self.phi = MLP(num_mlp_layers, input_dim * num_layers, hidden_dim, hidden_dim)        
        self.rho = MLP(num_mlp_layers, hidden_dim, hidden_dim, output_dim)        
    ...
    def forward(self, batch_graph):
        ...
        # do not use batch pooling first
        hidden_rep = [self.batch_norms[layer](h) for layer, h in enumerate(hidden_rep)]
        hidden_rep = torch.cat(hidden_rep, dim=1) # concat to get wider 

        rep = self.phi(hidden_rep)
        # import pdb; pdb.set_trace()
        result = self.rho(torch.spmm(graph_pool, rep))
